...
def test():
    ...
    name = 'AC_hard_Attention' # 模型参数保存的名字
...
# 激励函数
def A_func(x):
    return (10 / (1 + 9 * torch.exp(5 - 3 * x)))

# 输出转换函数
def func_out(out,x):
    return (x[:,0:1] - 1) * (x[:,0:1] + 1) * x[:,1:2] * out + (x[:,0:1] ** 2) * torch.cos(torch.pi * x[:,0:1])
...
def draw(pinn, load_path, device):
    ...
    with torch.no_grad():
        x = torch.arange(-1, 1.005, 0.005, device=device)  # 不包含最后一项
        t = torch.arange(0, 1.005, 0.005, device=device)
