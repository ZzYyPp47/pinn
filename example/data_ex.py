...
class create_point(object):
    ...
    def rand_point(self,delta):
        ...
        t = torch.arange(0, 1 + delta, delta, requires_grad=True,dtype=torch.float32,device=self.device)
        grid_x, grid_t = torch.meshgrid(x, t)
        self.pde_x = grid_x.reshape(-1, 1)
        self.pde_t = grid_t.reshape(-1, 1)
        self.attention = torch.zeros_like(self.pde_x,requires_grad=True,dtype=torch.float32,device=self.device)
