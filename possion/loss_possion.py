...
class LossCompute:
    def __init__(self, model, loss_func,loss_A_func ,point, A_func):
        self.model = model
        self.loss_func = loss_func
        self.loss_A_func = loss_A_func
        self.A_func = A_func
    ...
    def loss_pde(self):
        ...
        # 使用torch.nn.MSELoss计算MSE
        loss = self.loss_A_func(lapacian_u + 2 * (torch.pi ** 2) * torch.sin(torch.pi * self.point.pde_x) * torch.sin(torch.pi * self.point.pde_t), torch.zeros_like(u))
        # 应用自适应权重
        loss *= self.A_func(self.point.attention)
        return loss.mean()
