...
class create_point(object):
    ...
    def rand_point(self,delta):
        x = torch.arange(-2, 2 + delta, delta, requires_grad=True,dtype=torch.float32,device=self.device)  # 不包含最后一项
        t = torch.arange(-2, 2 + delta, delta, requires_grad=True,dtype=torch.float32,device=self.device)
