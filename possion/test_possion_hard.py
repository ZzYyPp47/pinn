...
from pinn_possion import *
from data_possion import *
...
def test():
    ...
    name = 'Possion_hard_Attention' # 模型参数保存的名字
...
# 输出转换函数
def func_out(out,x):
    return (x[:,0:1] - 2) * (x[:,0:1] + 2) * (x[:,1:2] - 2) * (x[:,1:2] + 2) * out
...
def draw(pinn, load_path, device):
    ...
    with torch.no_grad():
        x = torch.arange(-2, 2.05, 0.05, device=device)  # 不包含最后一项
        t = torch.arange(-2, 2.05, 0.05, device=device)
        ...
        u = (torch.sin(torch.pi * mesh_x) * torch.sin(torch.pi * mesh_t)).reshape(grid_x.shape)
        ...
        plt.figure()
        end = max(attention.reshape(-1,1)).item()
        start = min(attention.reshape(-1,1)).item()
        x = torch.arange(start,end,(end - 1)/25)  # 不包含最后一项
